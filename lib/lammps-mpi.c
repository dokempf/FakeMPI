...
{
  const char string[] = "FakeMPI (forked from LAMMPS MPI STUBS)";
  int len;
...
int
MPI_Get_version(int* major, int* minor)
{
  if (!major || !minor)
    return MPI_ERR_ARG;
...
static int
stubtypesize(MPI_Datatype datatype)
{
  if (datatype == MPI_INT)
    return sizeof(int);
    ...
    for (i = 0; i < nextra_datatype; i++)
      if (datatype == index_datatype[i])
        return size_datatype[i];
  }
  return 0;
}
...
int
MPI_Waitall(int n, MPI_Request* request, MPI_Status* status)
{
  static int callcount = 0;
  if (callcount == 0) {
    printf("MPI Stub WARNING: Should not wait on message from self\n");
...
{
  if (nextra_datatype == MAXEXTRA_DATATYPE)
    return -1;
